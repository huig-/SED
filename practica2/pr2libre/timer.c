...
void timers_init(void)
{
	...
	/*Timer1 conf*/
	rTCON = rTCON | (0x01<<9);    // timer1 manual_update
	rTCON = rTCON & ~(0x01<<9);   // timer1 disable manual_update
	rTCON = rTCON | (0x01<<11);    // timer1 auto-reload
	rTCON = rTCON | (0x01<<8);    // timer1 start
	/*Timer2 conf*/
...
void timer1_ISR(void)
{
	rINTMSK = rINTMSK & ~(BIT_TIMER4); //enable timer4
	DelayMs(500);
	rINTMSK = rINTMSK | BIT_TIMER1; //disable timer1
	rI_ISPC = BIT_TIMER1;
}
